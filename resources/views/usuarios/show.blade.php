...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Usuario #{{ $usuario->id }}</h1>
            <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm">{{ $usuario->nombre }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
            <h3 class="font-semibold text-slate-800 dark:text-slate-200">
                <i class="fas fa-info-circle me-2 text-indigo-500"></i> Datos del Usuario
            </h3>
        </div>
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <p class="text-slate-900 dark:text-white font-medium">{{ $usuario->nombre }}</p>
                </div>
            </div>
            <div>
            ...
            <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500 mb-1">Tipo de Usuario</p>
                @if($usuario->tipoUsuario)
                    <span class="inline-flex items-center px-3 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 font-medium text-sm">
                        <i class="fas fa-user-tag me-2 text-indigo-500 text-xs"></i>
                        {{ $usuario->tipoUsuario->nombre_tipo }}
                    </span>
                @else
                    <p class="text-slate-500 dark:text-slate-400">N/A</p>
                @endif
            </div>
        </div>
    </div>
