					...
					<div class="card">
						<div class="card-header bg-primary" style="font-size: 1.75rem; font-weight: 600; line-height: 1.2; margin-bottom: 0; color: white; display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <div style="display: flex; gap: 0.5rem; align-items: center;">
                                <a href="{{ route('usuarios.create') }}" class="btn btn-light" title="Nuevo Usuario" style="padding: 0.5rem 1rem; font-size: 0.9rem; color: #1E293B;">
							...
							<table id="example1" class="table table-bordered table-hover" style="width:100%">
								<thead style="background-color: var(--light);">
									<tr>
                                        ...
                                        <th style="font-weight: 600; color: #1E293B;">Tipo de Usuario</th>
									...
									<tr>
										...
										<td>
											<label class="toggle-switch-custom">
												<input data-type="usuario" data-id="{{$usuario->id}}" class="toggle-class" type="checkbox" {{ $usuario->estado ? 'checked' : '' }}>
												<span class="toggle-slider"></span>
											</label>
										</td>
										<td>
                                            <div style="display: flex; gap: 0.375rem;">
                                                <a href="{{ route('usuarios.show', $usuario) }}" class="btn btn-info btn-sm" title="Ver" style="padding: 0.375rem 0.625rem; min-width: 38px;">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="{{ route('usuarios.edit', $usuario) }}" class="btn btn-warning btn-sm" title="Editar" style="padding: 0.375rem 0.625rem; min-width: 38px;">
                                                    <i class="fas fa-pencil-alt"></i>
                                                </a>
                                                <form class="d-inline delete-form" action="{{ route('usuarios.destroy', $usuario) }}" method="POST" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
